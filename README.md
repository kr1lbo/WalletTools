...
- выполнять полный поиск приватных ключей на поддерживаемой NVIDIA GPU.
...
Сохраните EXE в отдельную папку с правом записи и запустите двойным щелчком. Для CPU-режима дополнительные компоненты не нужны. Для GPU-режима достаточно поддерживаемой NVIDIA GPU и актуального драйвера: Go и CUDA Toolkit пользователю устанавливать не требуется. Также доступен ZIP с программой, README, лицензией и уведомлениями о стороннем коде.
...
- Windows, Linux или macOS для CPU-режима
- NVIDIA GPU с архитектурой `sm_75` или новее и совместимый драйвер для GPU-режима
...
# Generate private-key candidates on an NVIDIA GPU.
# Mnemonic generation always remains on the CPU.
gpu_enabled: false
...
Для обычной однофайловой сборки оставьте `cuda_executable: "wallettools-cuda.exe"`:
если такого файла рядом нет, программа автоматически использует встроенный
движок. Другой путь в `cuda_executable` считается явным выбором внешнего helper.
...
GPU-режим применяется только к пункту **Generate by Private Keys**. При
`gpu_enabled: false` используется прежний CPU-генератор. При
`gpu_enabled: true` вычисление secp256k1, Keccak и сопоставление шаблонов
выполняются на GPU. CPU только управляет CUDA-процессами и независимо
перепроверяет уже найденный приватный ключ перед сохранением. Шифрование
keystore и запись результатов также остаются в основном процессе.

Готовый релиз является одним EXE. При первом GPU-запуске встроенный движок
извлекается в версионированный каталог `%LOCALAPPDATA%\WalletTools\cuda\`;
его SHA-256 проверяется перед использованием. Удаление этого кэша безопасно —
движок будет извлечён повторно.

Чтобы пересобрать встроенный helper из исходников на Windows, нужны CUDA Toolkit
13.x, Visual Studio 2022 Build Tools с MSVC C++ и Go 1.25 или новее:

```powershell
.\scripts\build-cuda.ps1
```

После этого пересоберите основной EXE командой `go build`. Для включения GPU измените:
...
Если движок не запускается, GPU или драйвер несовместимы либо указан неверный
внешний helper, генерация завершится ошибкой. Автоматического перехода на CPU
нет, чтобы программа не выдавала CPU-поиск за GPU-поиск. `cuda_batch_size`
управляет объёмом работы и расходом VRAM; рекомендуемое начальное значение —
`65536`. Настройка `cores` в полном GPU-режиме не создаёт CPU-поиск.

Полноценный CUDA backend поддерживает все уникальные шаблоны `specific` и
`symmetric` одновременно, включая связанные placeholders `X` и `Y`. Одинаковые
шаблоны объединяются. После совпадения шаблон с `final: false` автоматически
продолжает поиск, а совпадение `final: true` останавливает все CUDA-задачи.
Регулярные выражения также компилируются в CUDA-маски и выполняются на GPU.
Поддерживаются якоря `^`/`$`, литералы, `.`, классы (`[0-9a-f]`), группы,
альтернация `|` и квантификаторы `?`, `*`, `+`, `{n}`, `{n,m}`. Выражение
проверяется повторно на CPU только для найденного кандидата. Backreference
вида `\1`, lookahead/lookbehind и другие конструкции, отсутствующие в Go RE2,
не поддерживаются. Слишком сложное для развёртывания выражение завершается
понятной ошибкой. `edges` и регистрозависимый GPU-поиск пока не поддерживаются.
Если в конфиге присутствует активный `edges.minCount`, GPU-режим остановится с
ошибкой: установите `minCount: 0` или используйте CPU.

Каждая уникальная GPU-цель выполняется отдельным CUDA-процессом. При
`cuda_batch_size: 65536` одна цель обычно занимает около 2 ГиБ VRAM. Несколько
одновременных `specific`, `symmetric` и `regexp` суммируют расход памяти. При
ошибке `out of memory` уменьшите число целей или `cuda_batch_size`.

Подробные примеры и диагностика: [`docs/cuda.md`](docs/cuda.md).

### `configs/patterns.yaml`

Минимальный конфиг, совместимый одновременно с CPU и GPU:
...
Расширенный пример для CPU, включающий все типы паттернов:
...
Порядок проверки в CPU-режиме: `symmetric` → `specific` → `edges` → `regexp`; сохраняется первое совпадение. Поэтому более общий паттерн выше может перекрывать более узкий ниже. `final: true` останавливает генерацию после сохранения совпадения; уже найденные другими потоками результаты также могут попасть в файл. В GPU-режиме все уникальные поддерживаемые цели запускаются одновременно, а дубликаты объединяются.
...
Регулярные выражения применяются к полному адресу, включая `0x`. В примере
начальная серия должна содержать ровно 4–6 цифр, конечная серия — ровно 4–6
букв `a`–`f`; середина может быть любой. Отрицательные классы после и перед
сериями не позволяют им незаметно продолжиться сверх верхней границы.
...
GPU-компилятор поддерживает наиболее полезное подмножество Go regexp: якоря,
литералы, `.`, классы и отрицательные классы, группы, `|`, `?`, `*`, `+`,
`{n}` и `{n,m}`. CPU поддерживает весь синтаксис Go RE2. Если выражение нельзя
безопасно преобразовать в конечную CUDA-маску или оно вообще не ограничивает
hex-позиции, GPU-режим завершится понятной ошибкой.
...
```bash
git tag -a v1.1.0 -m "WalletTools v1.1.0"
git push origin v1.1.0
```

Перед публичным релизом CUDA helper должен быть собран с `./scripts/build-cuda.ps1 -Arch all`. Локальная сборка релиза в PowerShell: `./scripts/build-release.ps1 -Version v1.1.0`. Результаты находятся в `dist/v1.1.0/`. В архив включаются EXE, README, LICENSE и `THIRD_PARTY_NOTICES.md`; стартовые конфиги встроены в программу из `internal/portable/defaults/`, пользовательские конфиги, ключи и логи не упаковываются.
...
- модифицированный [ProVanity](https://github.com/WooMai/ProVanity) - CUDA backend; исходники находятся в `third_party/provanity`
...
Основной проект распространяется по `LICENSE`. Условия стороннего CUDA backend
приведены в `THIRD_PARTY_NOTICES.md` и `third_party/provanity/LICENSE`.
