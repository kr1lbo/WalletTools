...
- генерировать кандидаты приватных ключей на NVIDIA GPU через опциональный CUDA-helper.
...
- NVIDIA GPU и совместимый драйвер для GPU-режима
...
Чтобы собрать helper на Windows, установите [NVIDIA CUDA Toolkit](https://developer.nvidia.com/cuda-downloads), откройте новый PowerShell в каталоге проекта и выполните:

```powershell
.\scripts\build-cuda.ps1
```

CUDA-движок встраивается в `wallettools.exe`, поэтому пользователю не требуется
отдельный helper или установленный CUDA Toolkit. При первом запуске GPU-режима
движок автоматически извлекается в пользовательский каталог кэша. Затем измените:
...
Если helper отсутствует, имеет несовместимую архитектуру либо CUDA-драйвер не
работает, генерация завершится с понятной ошибкой — автоматического перехода на
CPU нет, чтобы ошибочно не считать GPU включённым. `cuda_batch_size` управляет
размером пакета; рекомендуемое начальное значение — `65536`. Настройка `cores`
в полном GPU-режиме не задаёт число генераторов и не создаёт CPU-поиск.
